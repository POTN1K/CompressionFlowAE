...
from keras.layers import InputSpec, Layer, MaxPool2D, AveragePooling2D
...
class MixedPooling2D(Layer):
    ...
    def _pooling_function(self, x, name=None):
        ...
        Mixed Pooling
        """
        max_pool = MaxPool2D(self.pool_size, padding=self.padding)(x)
        avg_pool = AveragePooling2D(self.pool_size, padding=self.padding)(x)
